  ...
  return (
    <li className={style.subtasks_li_container}>
      <div className={style.item_container}>
        <label>
          <input className={style.hidden_checkbox} type="checkbox" defaultChecked={subTask.completed}/>
      ...
      <div className={style.subtasks_tags_container}>
        {subTask.subTaskTags?.map((tag) => (
          <div style={{backgroundColor: tag.tagColor}} className={style.subtask_tag}>
            {tag.tagName}
          </div>
        ))}
        {subTask.subTaskTags && (
